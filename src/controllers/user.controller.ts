...
export const createUser = catchAsync(async (req: CustomRequest, res: Response) => {
  const { fullName, email, roleName } = req.body;
  const user = await userService.createUser({ fullName, email, roleName }, req.user!.company.id);
